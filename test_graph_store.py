...
class TestGraphStoreMigration:
    """Legacy FTS migration tests (covers _migrate_legacy_graph_fts lines 148-168)."""

    @pytest.mark.asyncio
    async def test_migrate_no_legacy_table(self, tmp_db_path):
        """Migration does nothing when legacy table does not exist."""
        store = GraphStore(tmp_db_path)
        await store.initialize()
        async with store._connect() as db:
            await store._migrate_legacy_graph_fts(db)

    @pytest.mark.asyncio
    async def test_migrate_new_fts_has_data(self, tmp_db_path):
        """Migration skipped when new FTS already has data."""
        store = GraphStore(tmp_db_path)
        await store.initialize()
        async with store._connect() as db:
            await db.execute(
                "CREATE TABLE livingmemory_graph_entries_fts (content TEXT, entry_id INTEGER)"
            )
            await db.execute(
                "INSERT INTO memora_graph_entries_fts(content, entry_id) VALUES (?, ?)",
                ("test content", 1),
            )
            await db.commit()
            await store._migrate_legacy_graph_fts(db)
    ...
    @pytest.mark.asyncio
    async def test_migrate_legacy_empty(self, tmp_db_path):
        """Migration skipped when legacy table has no rows."""
        store = GraphStore(tmp_db_path)
        await store.initialize()
        async with store._connect() as db:
            await db.execute(
    ...
    @pytest.mark.asyncio
    async def test_migrate_successfully_transfers_data(self, tmp_db_path):
        """Migration transfers data from legacy to new FTS table."""
        store = GraphStore(tmp_db_path)
        await store.initialize()
        async with store._connect() as db:
            await db.execute(
            ...
            await db.execute(
                "INSERT INTO livingmemory_graph_entries_fts(content, entry_id) VALUES (?, ?)",
                ("legacy content", 42),
            )
            await db.execute("DELETE FROM memora_graph_entries_fts")
            await db.commit()
            await store._migrate_legacy_graph_fts(db)

            cursor = await db.execute(
                "SELECT entry_id FROM memora_graph_entries_fts WHERE entry_id = ?", (42,)
            )
            row = await cursor.fetchone()
            ...
            cursor = await db.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='livingmemory_graph_entries_fts'"
            )
