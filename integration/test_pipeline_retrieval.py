...
class TestPipelineRetrieval:
    ...
    # ------------------------------------------------------------------
    # test_reranker_changes_result_order
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_reranker_changes_result_order(
        self,
        integration_engine: Any,
    ) -> None:
        """重排序验证：测试 reranker 组件存在性和接口契约。

        注意：integration_engine 默认配置 reranker.enabled=False，
        但 reranker 实例仍可验证接口。
        """
        engine = integration_engine
    ...
    # ------------------------------------------------------------------
    # test_retrieval_preserves_doc_structure
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_retrieval_preserves_doc_structure(
        self,
        integration_engine: Any,
    ) -> None:
        """验证 dual_route_retriever 组件挂载与接口完整性。

        检查所有检索链路上的关键组件存在且可调用。
        """
        engine = integration_engine

        # Assert — 核心检索组件存在
        if engine.dual_route_retriever is not None:
            assert hasattr(engine.dual_route_retriever, "search"), (
                "DualRouteRetriever 应有 'search' 方法"
            )
            assert callable(engine.dual_route_retriever.search), (
                "DualRouteRetriever.search 应可调用"
            )
            ...
            if hasattr(engine.dual_route_retriever, "graph_retriever"):
                graph_ret = engine.dual_route_retriever.graph_retriever
                if graph_ret is not None:
                    assert hasattr(graph_ret, "search"), (
                        "GraphRetriever 应有 'search' 方法"
                    )

        # Assert — hybrid_retriever 组件存在
        if engine.hybrid_retriever is not None:
            assert hasattr(engine.hybrid_retriever, "search"), (
                "HybridRetriever 应有 'search' 方法"
            )
            assert callable(engine.hybrid_retriever.search), (
                "HybridRetriever.search 应可调用"
            )
