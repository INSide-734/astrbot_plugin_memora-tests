...
def _make_fused(doc_id: int, rrf_score: float, content: str = "", metadata: dict | None = None) -> Any:
...
class TestScoreWeighting:
    ...
    def test_basic_weighting(self, weighting: Any) -> None:
        ...
        now = time.time()
        results = [
            _make_fused(1, 0.8, "test content", {
                "importance": 0.7,
    ...
    def test_sorting_by_final_score(self, weighting: Any) -> None:
        ...
        now = time.time()
        results = [
            _make_fused(1, 0.3, "low relevance", {
                "importance": 0.5, "create_time": now, "last_access_time": now,
            }),
            _make_fused(2, 0.9, "high relevance", {
                "importance": 0.9, "create_time": now, "last_access_time": now,
            }),
        ]
        output = weighting.apply_weighting(results, now)
    ...
    def test_temporal_decay(self, weighting: Any) -> None:
        ...
        recent_result = _make_fused(1, 0.5, "recent", {
            "importance": 0.5, "create_time": now, "last_access_time": now,
        })
        old_result = _make_fused(2, 0.5, "old", {
            "importance": 0.5, "create_time": old_time, "last_access_time": old_time,
        })
        output = weighting.apply_weighting(
            [recent_result, old_result], now,
        )
    ...
    def test_single_rrf_value(self, weighting: Any) -> None:
        ...
        now = time.time()
        results = [_make_fused(1, 0.0, "content", {
            "importance": 0.5, "create_time": now, "last_access_time": now,
        })]
        output = weighting.apply_weighting(results, now)
        assert len(output) == 1
