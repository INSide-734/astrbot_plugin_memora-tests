...
class TestIntentKeywords:
    ...
    def test_terms_are_tuples(self) -> None:
        ...
        from core.retrieval.intent_keywords import FACTUAL_TERMS, RELATION_TERMS, TEMPORAL_TERMS
        for coll in (RELATION_TERMS, TEMPORAL_TERMS, FACTUAL_TERMS):
            assert isinstance(coll, tuple)

    @pytest.mark.parametrize("query,expected_in_collection", [
        ("我和他是朋友", "RELATION"),
        ("昨天发生了什么", "TEMPORAL"),
        ("这个是什么东西", "FACTUAL"),
    ])
    def test_keyword_matching_in_queries(self, query: str, expected_in_collection: str) -> None:
