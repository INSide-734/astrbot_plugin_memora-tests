...
class TestAtomRetriever:
    ...
    @pytest.mark.asyncio
    async def test_search_returns_scored_atoms(
        self, retriever: Any, atom_store: AsyncMock
    ) -> None:
        ...
        assert (
            results[0].atom_id == 1
        )  # higher final_score (0.9*0.85=0.765 > 0.7*1.0=0.7)
        assert isinstance(results[0].final_score, float)

    @pytest.mark.asyncio
    async def test_search_default_metadata_scores(
        self, retriever: Any, atom_store: AsyncMock
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_search_passes_session_and_persona(
        self, retriever: Any, atom_store: AsyncMock
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_atoms_for_memory(
        self, retriever: Any, atom_store: AsyncMock
    ) -> None:
